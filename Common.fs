...
type ReactBuilder(import: obj) =
    ...
    [<CustomOperation("ref")>]
    member inline _.ref(s: DSLElement, v: HTMLElement -> unit) = s.attr "ref" v
    
    [<CustomOperation("spread")>]
    member _.spread(x: DSLElement, v: obj) =
        let attrs = 
            Constructors.Object.keys v
            |> Seq.map (fun key -> (key, v?(key)))
            |> Seq.toList
        
        { x with Attributes = x.Attributes @ attrs }
